...
前端、后端、Docker 及其他场景需要安装依赖包或第三方包时，必须优先使用国内镜像源。

- Node / pnpm / npm 依赖优先使用国内 npm 镜像源。
- Node.js 依赖安装必须使用 `pnpm`，不要使用 `npm install` 安装项目依赖。
- Docker 镜像拉取、基础镜像和系统包安装优先配置国内镜像源或国内加速源。
- Python 项目必须使用 `uv venv` 创建虚拟环境，并优先使用国内 Python 包镜像源安装依赖。
- 系统包管理器及其他语言生态依赖安装也应优先使用对应的国内镜像源。
- 如因镜像同步延迟或包缺失必须临时回退官方源，需要在最终回复或实施记录中说明原因、影响范围和已验证内容。
